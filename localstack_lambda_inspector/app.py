...
# Endpoint URL
ENDPOINT_URL = "http://localhost:4566/_extensions/lambda-inspector/invocations?formatted=true"
...
max_invocations = st.sidebar.number_input("Show last X invocations", min_value=1, value=20, step=1,
                                          key="max_invocations")
...
if data and "invocations" in data:
    ...
    if not invocations:
        ...
        if not filtered_invocations:
            ...
            for invocation in display_list:
                ...
                expander_label = f"{status_icon} | {timestamp} | {function_name} | {request_id}"
    ...
    st.warning("Could not retrieve invocations data. Make sure LocalStack is running and the endpoint is accessible.")
